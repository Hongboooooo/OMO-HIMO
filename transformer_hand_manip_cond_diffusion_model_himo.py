...
class TransformerDiffusionModel(nn.Module):
    ...
    def forward(self, src, noise_t, condition_A, condition_B, padding_mask=None):
        ...
        src = torch.cat((src, condition_A, condition_B), dim=-1) # concatenate along last dimension | BS X T X (2 x 3 + 3 + 256)
        ...
        feat_pred, _ = self.motion_transformer(data_input, padding_mask, pos_vec, obj_embedding=noise_t_embed) # feat_pred: (bs, 121, 512)
...
class CondGaussianDiffusion(nn.Module):
    def __init__(
        ...
        self.bps_encoder_A = nn.Sequential(
            nn.Linear(in_features=1024*3, out_features=512),
            nn.ReLU(),
            nn.Linear(in_features=512, out_features=256),
            )
        
        self.bps_encoder_B = nn.Sequential(
            nn.Linear(in_features=1024*3, out_features=512),
            nn.ReLU(),
            nn.Linear(in_features=512, out_features=256),
            )
        ...
        obj_feats_dim = 256 
        d_input_feats = d_feats+3+obj_feats_dim+3+obj_feats_dim # 2x3 + 3 + 256
    ...
    def p_losses(self, x_start, x_cond_A, x_cond_B, t, noise=None, padding_mask=None):
        ...
        model_out = self.denoise_fn(x, t, x_cond_A, x_cond_B, padding_mask) # x is noised input
    ...
    def forward(self, x_start, obj_A_cond, obj_B_cond, cond_mask=None, padding_mask=None):
        # x_start: BS X T X D
        ...
        if cond_mask is not None: # cond_mask is None by default
            x_pose_cond_A = x_start * (1. - cond_mask) + cond_mask * torch.randn_like(x_start).to(x_start.device)
            x_cond_A = torch.cat((x_cond_A, x_pose_cond_A), dim=-1)
            x_pose_cond_B = x_start * (1. - cond_mask) + cond_mask * torch.randn_like(x_start).to(x_start.device)
            x_cond_B = torch.cat((x_cond_B, x_pose_cond_B), dim=-1)
        curr_loss = self.p_losses(x_start, x_cond_A, x_cond_B, t, padding_mask=padding_mask) # cond_mask is None; padding_mask is Not None by default
